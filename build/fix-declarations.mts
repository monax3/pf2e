...
async function fixImports(files: string[]): Promise<void> {
    const aliases: [RegExp, string][] = [
        [new RegExp(/['"][\./]*types\/foundry\/(.*?)['"]/g), "./types/foundry/$1"],
        [new RegExp(/['"][\./]*static\/lang\/en.json['"]/g), "./types/pf2e/en.json"],
        ...getAliases()
    ];
    ...
    for (const file of files) {
        ...
        data = data.replaceAll(/declare module ['"]@client\/helpers\/hooks.mjs['"]/g, `declare module "pf2e-types/hooks"`);

        for (const [regex, into] of aliases) {
            data = data.replaceAll(regex, (_, repl: string) => {
                if (into.startsWith('pf2e-types')) return `"${into.replace("$1", repl)}"`;
...
async function afterBuild(): Promise<void> {
    ...
    const pf2eFiles = globSync("./types/pf2e/**/*.{m,}ts");
    const typeFiles = globSync("./types/**/*.{m,}ts");

    await fixImports(typeFiles);
